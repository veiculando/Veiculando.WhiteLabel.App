  ...
  template: `
    <div class="inventory-workspace">
      <aside id="inventory-results" class="results-panel" [class.sheet-expanded]="sheetExpanded()" aria-label="Peças encontradas">
        ...
        <div class="result-heading"><strong><span class="result-count-desktop">{{ points().length }} {{ points().length === 1 ? 'peça encontrada' : 'peças encontradas' }}</span><span class="result-count-mobile">{{ points().length }} {{ points().length === 1 ? 'peça próxima' : 'peças próximas' }}</span></strong><button class="sheet-toggle" type="button" [attr.aria-expanded]="sheetExpanded()" aria-controls="inventory-results" (click)="sheetExpanded.set(!sheetExpanded())">{{sheetExpanded()?'Ver mapa':'Mais filtros ⌄'}}</button></div>
